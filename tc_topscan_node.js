...
const enableSerialResponse = true; // true: 启用串口应答 / enable serial response, false: 禁用 / disable

const debugmode = 0; // 0: 日志 / VNLib.Log, 1: 外部输出 / sendOutput, 2: 两者都输出 / both

// 历史超时清理开关 / Timeout cleanup switch for stored history
const enableTimeoutClear = true; // true: 启用超时清理 / enable timeout cleanup, false: 禁用 / disable
const timeoutSeconds = 10; // 超时时间（秒）/ Timeout time (seconds)

// 无码输出控制开关 / No-barcode output control switch
const enableNoBarcodeOutput = false; // true: 无码不输出 / no output when no barcode, false: 无码输出 ???? / output ????
...
let bypassMode = GlobalString.load("bypassMode") === "true"; // false: 普通模式 / normal, true: bypass 模式 / bypass
...
function handleSerialMessage(message) {
    ...
    if (trimmedMessage === "bypass on") {
        bypassMode = true;
        GlobalString.store("bypassMode", "true"); // 保存到全局变量 / Save to global variable
        ...
        bypassMode = false;
        GlobalString.store("bypassMode", "false"); // 保存到全局变量 / Save to global variable
...
const useTimeInMetadata = true;        // 是否包含时间 / Whether to include time in metadata
const useAngleInMetadata = true;       // 是否包含角度 / Whether to include angle in metadata
const useDeviceInMetadata = false;      // 是否包含设备信息 / Whether to include device information in metadata
const useBoxPassLineInMetadata = true;  // 是否包含包裹过线信息 / Whether to include box pass line information
const useCameraStatusInMetadata = true; // 是否包含相机状态 / Whether to include device status in metadata
const useSideRoiModeInMetadata = true; // 是否包含侧扫 ROI 模式 / Whether to include side ROI mode in metadata
const useParcelEdgeLengthInMetadata = true; // 是否包含包裹边长 / Whether to include parcel edge length in metadata
...
function convertToISO8601Zulu(timeValue) {
    ...
    try {
        ...
        if (timeValue.includes('/') && timeValue.includes(':')) {
            ...
            if (parts.length >= 2) {
                const datePart = parts[0]; // 日期部分 / date part, e.g. "2025/06/19"
                const timePart = parts[1]; // 时间部分 / time part, e.g. "17:15:11:195"
...
function getNextJobId() {
    ...
    // 检查包裹角度是否缺失（设备号允许为空）/ Check if box_angle is missing (device_number may be empty)
    ...
    if (nextId === 0) nextId = 1; // 确保从 1 开始 / Ensure starting from 1
...
function filterCodesByRoi(codeStr, centerStr, roiNumberStr) {
    try {
        ...
        // 使用 ROI 模式，返回 Tall 或 Short / Use ROI mode which returns Tall or Short
        const roiMode = judgeRoiMode();
        ...
        for (let i = 0; i < codeArr.length; i++) {
            ...
            // ROI_number 为空或未定义时视为无 ROI，跳过 / undefined or empty ROI_number means no ROI, skip
            if (!rawRoi) {
                continue;
            }
            ...
            // -1 表示未启用 ROI，跳过 / -1 means ROI not enabled, skip
            if (roiIdx === -1) {
                continue;
            }
        ...
        if (filteredCodes.length === 0) {
            // ROI 已启用但没有条码落入目标 ROI / ROI is enabled and no barcode falls into target ROI:
            // 返回空结果，让后续走无码输出流程 / Return empty result so downstream follows no-barcode output flow.
...
function parseMaxicode(maxicodeData) {
    ...
    // 直接返回原始数据，不做处理 / Directly return original data, no processing
    return cleanData;
}
...
function getDisposalMark(oneZCodes, specialOneDCodes, maxiCodes, postalCodes) {
    // 1Z 和特殊一维码在多码冲突中视为同一类 1D-like / 1Z and special 1D are treated as one 1D-like category for multi-code conflicts.
    const oneDLikeCount = oneZCodes.length + specialOneDCodes.length;
    if (oneDLikeCount > 1 || maxiCodes.length > 1) {
        return "2"; // 同类多个条码 / Multiple barcodes in one category detected
    }
    
    // 检查是否只有 PostalCode（视为未读到有效主码）/ Check if only postal codes exist (considered unread)
    if (oneDLikeCount === 0 && maxiCodes.length === 0) {
        return "1"; // 未读到有效主码 / No valid main barcode read
    }
    
    // 检查是否只有一种主码（不完整）/ Check if only one main barcode type exists (incomplete)
    if ((oneDLikeCount === 1 && maxiCodes.length === 0) || 
        (oneDLikeCount === 0 && maxiCodes.length === 1)) {
        return "1"; // 条码组合不完整 / Incomplete barcode set
    }
    
    // Check if only postal codes exist (incomplete)
    if (oneDLikeCount === 0 && maxiCodes.length === 0 && postalCodes.length > 0) {
        return "1"; // 只有 PostalCode（不完整）/ Only postal codes exist (incomplete)
    }
    
    return "0"; // 单包裹正常结果 / Single package detected
}
...
function formatOutput(messageType, oneZCode, maxicodeData, postalCode, disposalMark, metadata) {
    ...
    // Maxicode 原样输出，不再拆分 / Use Maxicode data directly, no splitting
    ...
    return [
        ...
        postalCode || "",
        disposalMark,
        "0", // 固定附加字段 / Fixed additional field
        metadata
    ].join('\t') + endStr;
}
...
function checkAndClearTimeoutHistory() {
    ...
    try {
        ...
        // 获取条码时间记录 / Get barcode time records
        ...
        // 检查每个条码是否超时 / Check whether each barcode timed out
        const validHistory = [];
        ...
        for (let i = 0; i < history.length; i++) {
            ...
            for (const code of codes) {
                ...
                if (codeTime) {
                    const timeDiff = (currentTime - codeTime) / 1000; // 转为秒 / Convert to seconds
                    ...
                    if (timeDiff >= timeoutSeconds) {
                        ...
                        // 保留未超时的条码时间记录 / Keep non-timeout barcode time records
        ...
        // 更新历史记录和时间记录 / Update history and time records
        if (historyCleared) {
...
function storeHistory(codes) {
    // 检查超时并清理历史 / Check timeout and clear history
    checkAndClearTimeoutHistory();
    
    // 读取当前历史记录 / Read current history records
    ...
    // 超过 4 条时删除最旧记录 / If more than 4 records, remove the oldest
    if (history.length > 4) {
        history.shift();
    }
    
    // 记录每个条码的识别时间 / Record recognition time for each barcode
    const currentTime = Date.now();
    const codeTimesData = GlobalString.load("codeTimes");
    const codeTimes = codeTimesData ? JSON.parse(codeTimesData) : {};
    
    // 为每个条码记录当前时间 / Record current time for each barcode
    for (const code of codes) {
        codeTimes[code] = currentTime;
        logDebug(`Record barcode ${code} recognition time: ${new Date(currentTime).toISOString()}`);
    }
    
    // 保存更新后的历史和时间记录 / Save updated history and time records
...
function isDuplicate(code) {
    ...
    // 遍历所有历史记录 / Check all history records
...
function isSpecialOneDCode(code) {
    // B 或 1B 开头的条码 / Barcode starting with B or 1B
...
function extractSpecialOneDFromMaxicode(maxicode) {
    ...
    try {
        // 定义 ASCII 控制字符（真实分隔符）/ Define ASCII control characters (true separators)
        // GS 组分隔符 = 0x1D = 29 / GS (Group Separator) = 0x1D = 29
        // RS 记录分隔符 = 0x1E = 30 / RS (Record Separator) = 0x1E = 30
        const GS_ASCII = String.fromCharCode(0x1D);  // ASCII 29 / GS 真实控制字符
        const RS_ASCII = String.fromCharCode(0x1E);  // ASCII 30 / RS 真实控制字符
        
        // 定义可视化符号（字符串中可能出现的 Unicode 符号）/ Define visual symbols (Unicode symbols that may appear in strings)
        // 这些是 Unicode 可视化符号，不是真实控制字符 / These are Unicode symbols, not actual control characters
        const GS_VISUAL = "␝";  // Unicode U+241D / GS 可视化符号
        const RS_VISUAL = "␞";  // Unicode U+241E / RS 可视化符号
        
        // 如果存在前缀 [)> 则移除 / Remove the leading [)> if present
        let maxicodeData = maxicode;
        if (maxicodeData.startsWith("[)>")) {
            maxicodeData = maxicodeData.substring(3);
        }
        
        // 关键步骤：把可视化符号替换为真实 ASCII 控制字符 / Key step: replace visual symbols with actual ASCII control characters
        // 兼容字符串里是 Unicode 可视化符号而非真实控制字符的情况 / Handles Unicode visual symbols instead of control chars
        maxicodeData = maxicodeData.replace(new RegExp(RS_VISUAL, 'g'), RS_ASCII)
                                   .replace(new RegExp(GS_VISUAL, 'g'), GS_ASCII);
        
        // 使用真实 ASCII GS/RS 分隔符拆分 / Split by both GS and RS using actual ASCII control characters
        ...
        // 过滤拆分后的空字段 / Filter out empty strings from split
        ...
        if (nonEmptyFields.length > 4) {
            ...
            // 检查第 5 个字段是否以 B 或 1B 开头 / Check if field 5 starts with B or 1B
...
function checkBoxAndLineIntersection() {
    ...
    // ------------------- 内部工具：获取矩形四条标准边（不依赖顶点顺序）/ Internal helper: get four standard rectangle edges independent of vertex order -------------------
    ...
    // ------------------- 内部工具：判断两条线段是否相交 / Internal helper: determine if two line segments intersect -------------------
    ...
    // ------------------- 内部工具：判断点是否在矩形内（带浮点容差）/ Internal helper: determine whether a point is inside rectangle with tolerance -------------------
    ...
    // 可选：返回相交数量，便于外部调用 / Optional: return intersection count for external calls
