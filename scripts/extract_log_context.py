...
DEFAULT_MARKERS = {
    "ERROR": ["error", "exception", "failed", "uncaught", "timeout", "not found", "invalid", "错误", "异常", "失败", "超时"],
    "TASK": ["taskid", "task id", "task state switch", "TS_WAITTRI", "TS_ALGODONE", "TS_GROUP_DONE", "任务", "触发"],
    "TRIGGER": ["trig", "trigger", "missed trigger", "io trig", "forcibly end", "触发", "漏触发"],
    "DECODE": ["begin algorithm run decode", "finish algorithm run decode", "begin algorithm sdk run", "finish algorithm sdk run", "decode", "algorithm", "识别", "算法"],
    "SCRIPT": ["script", "js log", "vnlib", "processcodes", "jsscript", "uncaught exception at line", "=== start processing barcodes ===", "input parameters", "generated task id", "bypass mode", "disposal mark", "only found", "multiple barcodes", "final selection", "callback function execution error", "registercallback call success", "脚本"],
    "OUTPUT": ["camera.postscan", "sendoutput", "sendresult", "outputstring", "formatOutput", "output ???", "output !!!!", "输出"],
    "ROI": ["roi", "roi_number", "roimode", "getslaveroiindex", "RoiIndex", "slave roi", "set roi index", "set all roi", "Final selection", "callback SetRoiIndex returned", "Tall", "Short", "高箱", "矮箱"],
    "GROUP": ["group", "slave", "client", "host recv", "ResulReady", "add slave roi index", "result recved slave", "组网", "从机", "主机"],
    "HISTORY": ["lasttaskcodes", "codetimes", "history", "duplicate", "timeout clear", "storehistory", "isduplicate", "updated stored barcodes", "history barcodes", "历史", "重复", "去重"],
    "COMP": ["missedtrigger", "missed trigger", "compensation", "GetMissedTriggerCountDuringTask", "failed to get missed trigger count", "漏触发", "补偿"],
    "CMD": ["strstored", "strtcpstored", "m_tcpScriptStrIn", "m_ScriptStrIn", "bypass", "resetnum", "resetno", "命令"],
    "CONFIG": ["undefined", "referenceerror", "nan", "not a callable function", "callback", "is empty", "变量", "映射", "配置", "未定义"],
}
...
TAG_ADVICE = {
    ...
    "OUTPUT": "优先检查最终输出分支、输出字段入参、是否被历史去重/ROI 过滤改成 ????。",
...
def issue_keywords(issue: str) -> List[str]:
    words = re.findall(r"[A-Za-z0-9_?.!-]{2,}|[\u4e00-\u9fff]{2,}", issue or "")
    ignored = {"the", "and", "with", "this", "that", "问题", "日志", "任务"}
    return [w for w in words if w.lower() not in ignored][:12]
...
def line_matches(line: str, needles: Sequence[str]) -> bool:
    ...
    for needle in needles:
        ...
        if needle.lower() in lower:
            return True
        compact = normalize_token(needle)
...
def classify_line(line: str) -> List[str]:
    lower = line.lower()
    return [tag for tag, markers in DEFAULT_MARKERS.items() if any(m.lower() in lower for m in markers)]


def print_advice(tag_counts):
    print("=== NEXT CHECKS ===")
    if not tag_counts:
...
def main() -> int:
    ...
    lines = read_text(args.log_file).splitlines()
    if not lines:
    ...
    needles: List[str] = []
    if args.task_id:
        needles.extend([args.task_id, f"TaskId:{args.task_id}", f"taskid:{args.task_id}"])
    needles.extend(args.keyword)
    needles.extend(issue_keywords(args.issue))

    matches = [i for i, line in enumerate(lines) if line_matches(line, needles)] if needles else []
    if not matches:
    ...
    ranges = merge_ranges(((i - args.window, i + args.window + 1) for i in matches), len(lines))
    ...
    for start, end in ranges:
        for idx in range(start, end):
            ...
            if tags:
                ...
                print(f"{idx + 1}: [{' '.join(tags)}] {lines[idx]}")
                highlight_count += 1
                if highlight_count >= 160:
                    print("... highlight output truncated")
                    break
        if highlight_count >= 160:
            break
