...
本文说明当前有效顶扫脚本 `tc_in_0401.js` 在下位机中的执行时序，重点回答以下问题：
...
本文基于更新后的 `tc_in_0401.js` 整理。侧扫脚本当前按用户要求先忽略，只把 `VNLib.GetSlaveRoiIndex()` 作为顶扫可选输入背景。

## 2. 脚本文件分工

当前维护重点：

- `tc_in_0401.js`：当前有效顶扫输出脚本，用于输入快照日志、命令处理、metadata、ROI 过滤、条码分类、历史去重、输出决策和漏触发补偿。
- `tc_out_0401.js`：如果现场仍要求两个顶扫脚本一致，可按 `tc_in_0401.js` 同步；本时序文档不重复展开。
...
本轮文档先忽略侧扫脚本。更新后的顶扫 `tc_in_0401.js` 不再定义或调用 `SetRoiIndex/RegisterCallback`。
...
4. 首行输出 `InputSnapshot`，记录所有关键输入信息。
...
这一阶段非常重要，因为顶扫脚本后续会基于本次注入变量生成 `InputSnapshot` 和输出结果。
...
`processCodes()` 的第一步是创建 RuntimeContext，并立即调用 `logInitialInputSnapshot(context)`。

第一条业务日志格式类似：

```text
InputSnapshot | version=2026-04-27 | code=... | center=... | ROI_number=... | time=... | strStored=... | strTcpStored=... | box_coordinate=... | device_number=... | box_angle=... | is_box_pass_line=...
```
...
当前 `tc_in_0401.js` 不应再出现 `SetRoiIndex()`、`RegisterCallback()` 或主动 `RegisterCallback();`。如果出现这些函数，说明使用的不是当前重构后的顶扫版本。
...
顶扫识别结果
  ...
  -> tc_in_0401.js createRuntimeContext
  -> InputSnapshot 首行日志
  -> 命令处理 / metadata
...
| 输入快照 | 顶扫脚本 | `logInitialInputSnapshot()` | `InputSnapshot` |
...
| 顶扫输出 | `tc_in_0401.js` | `emitPostScan()`、`VNLib.SendOutput()` | `Camera.PostScan` |
...
当前顶扫脚本第一条业务日志应为 `InputSnapshot`。如果没有看到该日志，优先确认是否已粘贴更新后的 `tc_in_0401.js`。
...
1. 查看第一条业务日志是否为 `InputSnapshot`，确认 `code/center/ROI_number/time` 等输入值正确。
...
当前 `tc_in_0401.js` 不是单纯的字符串格式化脚本，而是承担了完整顶扫输出决策：

- 使用 RuntimeContext 组织单次任务数据。
- 首行输出 `InputSnapshot`，方便现场排查变量注入。
- 可选读取 ROI 状态并过滤条码。
