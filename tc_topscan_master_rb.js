...
function emitIncompletePlaceholder(disposalMark, metadata, enabledLog, disabledLog) {
    if (!enableNoBarcodeOutput) {
        sendOutput(formatOutput("PostScan", "????", "????", "????", disposalMark, metadata));
        logDebug(enabledLog);
    } else {
        logDebug(disabledLog);
    }
}
function handleSingleSpecialOneDIncomplete(previousOutput, specialOneDCodes, postalCodes, disposalMark, metadata) {
...
function handleSingleOneZIncomplete(previousOutput, oneZCodes, postalCodes, disposalMark, metadata) {
...
function handleSingleMaxicodeIncomplete(previousOutput, qrCodes, postalCodes, disposalMark, metadata) {
...
function handleIncompleteOutput(oneZCodes, qrCodes, specialOneDCodes, postalCodes, previousOutput, disposalMark, metadata) {
    ...
    if (oneZCodes.length === 0 && qrCodes.length === 0 && specialOneDCodes.length === 1) {
        handleSingleSpecialOneDIncomplete(previousOutput, specialOneDCodes, postalCodes, disposalMark, metadata);
        return;
    } else if (oneZCodes.length === 1 && qrCodes.length === 0) {
        handleSingleOneZIncomplete(previousOutput, oneZCodes, postalCodes, disposalMark, metadata);
        return;
    } else if (oneZCodes.length === 0 && qrCodes.length === 1) {
        handleSingleMaxicodeIncomplete(previousOutput, qrCodes, postalCodes, disposalMark, metadata);
        return;
    } else if (oneZCodes.length === 0 && qrCodes.length === 0 && postalCodes.length > 0) {
        emitIncompletePlaceholder(
            disposalMark,
            metadata,
            "Only found postal codes, all fields use ???? to represent",
            "Only found postal codes, no barcode output switch enabled, no output"
        );
        return;
    } else {
        emitIncompletePlaceholder(
            disposalMark,
            metadata,
            "No barcodes read, all fields use ???? to represent",
            "No barcodes read, no barcode output switch enabled, no output"
        );
        return;
    }
}
function processCodes() {
    ...
    if (disposalMark === "1") {
        handleIncompleteOutput(oneZCodes, qrCodes, specialOneDCodes, postalCodes, previousOutput, disposalMark, metadata);
        return;
    }
    if (disposalMark === "2") {
...
function buildParcelEdges(targetBox) {
    return [
...
function getTargetParcelGeometry() {
    ...
    return {
        ...
        edges: buildParcelEdges(targetBox),
    };
}
function getParcelAngle() {
    try {
        /* ===== 1. Read core engine data: parcel coordinates, movement direction and print debug logs ===== */
        var parcelGeometry = getTargetParcelGeometry();
        var boxData = parcelGeometry ? parcelGeometry.boxData : [];
        var direction = parcelGeometry ? parcelGeometry.direction : undefined;
        ...
        var targetBox = parcelGeometry.targetBox;
        var targetBoxCenter = parcelGeometry.targetBoxCenter;
...
function getParcelEdgeLength() {
    try {
        ...
        var edges = parcelGeometry.edges;
